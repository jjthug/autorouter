...
export function computeAllRiverexRoutes(
  tokenIn: Token,
  tokenOut: Token,
  pools: RiverexPair[],
  maxHops: number
): RiverexRoute[] {
  return computeAllRoutes<RiverexPair, RiverexRoute>(
    tokenIn,
    tokenOut,
    (route: RiverexPair[], tokenIn: Token, tokenOut: Token) => {
      return new RiverexRoute(route, tokenIn, tokenOut);
    },
    pools,
    maxHops
  );
...
export function computeAllRoutes<
  TPool extends Pair | Pool | RiverexPair,
  TRoute extends V3Route | V2Route | MixedRoute | RiverexRoute
>(
  tokenIn: Token,
  tokenOut: Token,
