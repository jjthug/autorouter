...
export async function getBestSwapRoute(
  ...
  console.log("starting to calculate best swap route");
...
> {
  ...
  const estimatedGasUsedUSDs = _(bestSwap)
    .map((routeWithValidQuote) => {
      const gasTokenDecimals = routeWithValidQuote.gasCostInUSD.currency.decimals;
      const decimalsDiff = usdTokenDecimals - gasTokenDecimals;

      if (decimalsDiff >= 0) {
        // Adjusting for decimal difference
        const adjustedQuotient = JSBI.multiply(
        ...
        // Adjusting for decimal difference
        const adjustedQuotient = JSBI.divide(
          routeWithValidQuote.gasCostInUSD.quotient,
          JSBI.exponentiate(JSBI.BigInt(10), JSBI.BigInt(-decimalsDiff))
        );
        return CurrencyAmount.fromRawAmount(
