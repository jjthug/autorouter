...
import {IRiverexPoolProvider} from '../providers';
import {
  ArbitrumGasData,
  OptimismGasData,
...
import {ChainId, log, Protocol, WRAPPED_NATIVE_CURRENCY} from '../util';
...
export function initSwapRouteFromExisting(
  ...
  const routesWithValidQuote = swapRoute.route.map((route) => {
    switch (route.protocol) {
      ...
      case Protocol.RIVERDEX:{
        return new RiverexRouteWithValidQuote({
          amount: CurrencyAmount.fromFractionalAmount(
            route.amount.currency,
            route.amount.numerator,
            route.amount.denominator
          ...
          tradeType: tradeType,
          riverexPoolProvider: riverexPoolProvider,
        });
      }
    }
  });
