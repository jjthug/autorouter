...
export class CachingTokenProviderWithFallback implements ITokenProvider {
  ...
  public async setTokens(pools: RiverexPool[]) : Promise<void>{
    await Promise.all(pools.map(({firstToken, secondToken}) => {
      Promise.all(
      [this.tokenCache.set(this.CACHE_KEY(this.chainId, firstToken.address.toLowerCase()),new Token(firstToken.chainId,firstToken.address,firstToken.decimals,firstToken.symbol)),
      this.tokenCache.set(this.CACHE_KEY(this.chainId, secondToken.address.toLowerCase()),new Token(secondToken.chainId,secondToken.address,secondToken.decimals,secondToken.symbol))])
    }))
  }
}
